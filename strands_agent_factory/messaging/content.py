...
def is_likely_text_file(file_path: PathLike) -> bool:
    ...
    if not path.exists() or not path.is_file():
        logger.debug(
            "is_likely_text_file returning False (file does not exist or not regular file)"
        )
        return False
    ...
    if path.suffix.lower() in text_extensions:
        logger.debug(
            "is_likely_text_file returning True (known text extension: {})",
            path.suffix.lower(),
        )
        return True
    ...
    if not path.suffix:
        ...
        if path.name.lower() in common_text_names:
            logger.debug(
                "is_likely_text_file returning True (common text filename: {})",
                path.name.lower(),
            )
            return True
    ...
    try:
        if path.stat().st_size > 1024 * 1024:  # Skip files larger than 1MB
            logger.debug(
                "is_likely_text_file returning False (file too large: {} bytes)",
                path.stat().st_size,
            )
            return False
        ...
        try:
            ...
            try:
                ...
                result = ratio > 0.7
                logger.debug(
                    "is_likely_text_file returning {} (printable ratio: {})",
                    result,
                    ratio,
                )
                return result
...
def paths_to_file_references(
    file_paths: List[Tuple[PathLike, Optional[str]]],
) -> List[str]:
    """
    ...
    if logger.level("TRACE").no >= logger._core.min_level:
        logger.trace(
            "paths_to_file_references called with {} file paths", len(file_paths)
        )
...
def recursively_remove(
    ...
    """
    logger.trace(
        "recursively_remove called with obj type={}, key_to_remove='{}'",
        type(obj).__name__,
        key_to_remove,
    )
...
def load_structured_file(
    ...
    """
    logger.trace(
        "load_structured_file called with file_path='{}', file_format='{}'",
        file_path,
        file_format,
    )
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            ...
            logger.debug(
                "load_structured_file returning config with {} top-level keys",
                len(result),
            )
            return result
...
def load_file_content(
    ...
    """
    logger.trace(
        "load_file_content called with file_path='{}', content_type='{}'",
        file_path,
        content_type,
    )
    ...
    try:
        if content_type == "text":
            with open(path, "r", errors="replace") as f:
                content = f.read()
                logger.debug(
                    "load_file_content returning text content ({} chars)", len(content)
                )
                return content
        else:
            with open(path, "rb") as f:
                content = f.read()
                logger.debug(
                    "load_file_content returning binary content ({} bytes)",
                    len(content),
                )
                return content
    except OSError as e:
...
def generate_file_content_block(
    file_path: Path, mimetype: str
    ...
    """
    logger.trace(
        "generate_file_content_block called with file_path='{}', mimetype='{}'",
        file_path,
        mimetype,
    )
    ...
    if file_path.stat().st_size > MAX_FILE_SIZE_BYTES:
        logger.warning(
            f"Skipping file '{file_path}' because it exceeds the {MAX_FILE_SIZE_BYTES / (1024*1024):.0f}MB size limit."
        )
        result = {
            "type": "text",
    ...
    detected_mimetype = (
        mimetype or guess_mimetype(file_path) or "application/octet-stream"
    )
    ...
    logger.debug(
        "File analysis: mimetype={}, format={}, likely_text={}, size={}",
        detected_mimetype,
        format,
        likely_text,
        len(file_bytes),
    ...
    if detected_mimetype.startswith("video/"):
        result = {"video": {"format": format, "source": {"bytes": file_bytes}}}
        logger.debug("generate_file_content_block returning video block")
        return result

    # Image files
    if detected_mimetype.startswith("image/"):
        result = {"image": {"format": format, "source": {"bytes": file_bytes}}}
...
def files_to_content_blocks(
    files: List[tuple[str, str]], max_files: Optional[int] = None
) -> List[Dict[str, Any]]:
    """
    ...
    if logger.level("TRACE").no >= logger._core.min_level:
        logger.trace(
            "files_to_content_blocks called with {} files, max_files={}",
            len(files),
            max_files,
        )
    ...
    logger.debug(
        "files_to_content_blocks returning {} content blocks", len(content_blocks)
    )
