...
def print_structured_data(
    data: Any,
    indent_level: int = 0,
    initial_max_len: int = MAX_VALUE_LENGTH,
    printer=print,
):
    """
    ...
    """
    logger.trace(
        "print_structured_data called with data type: {}, indent_level: {}, initial_max_len: {}",
        type(data).__name__,
        indent_level,
        initial_max_len,
    )
    ...
    def _format_and_print_value(value_to_format: Any, prefix: str = ""):
        ...
        logger.trace(
            "_format_and_print_value called with value type: {}, prefix: '{}'",
            type(value_to_format).__name__,
            prefix,
        )
        ...
        if value_to_format is None:
            ...
            print_structured_data(
                value_to_format, indent_level + 1, initial_max_len, printer
            )
            ...
            if (
                effective_max_len != float("inf")
                and len(value_as_str) > effective_max_len
            ):
