...
class AgentProxy:
    ...
    def __init__(
        ...
        tool_specs: List[EnhancedToolSpec],
        **kwargs: Any,
    ) -> None:
        ...
        if logger.level("TRACE").no >= logger._core.min_level:
            logger.trace(
                "AgentProxy.__init__ called with {} tool specs", len(tool_specs)
            )
        ...
        self._local_tool_specs = [
            obj for obj in tool_specs if not obj.get("client") and not obj.get("error")
        ]
        self._mcp_client_specs = [
            obj for obj in tool_specs if obj.get("client") and not obj.get("error")
        ]
        ...
        if logger.level("TRACE").no >= logger._core.min_level:
            logger.trace(
                "AgentProxy.__init__ completed - {} MCP clients, {} regular tools",
                len(self._mcp_client_specs),
                len(self._tools),
            )
    ...
    def __enter__(self):
        ...
        if logger.level("TRACE").no >= logger._core.min_level:
            logger.trace(
                "AgentProxy.__enter__ called with {} MCP server specs",
                len(self._mcp_client_specs),
            )
        ...
        if self._mcp_client_specs:
            ...
            for client in [spec["client"] for spec in self._mcp_client_specs]:
                try:
                    ...
                    logger.debug(
                        "Could not register cleanup for client {}: {}",
                        getattr(client, "server_id", "unknown"),
                        e,
                    )
            ...
            with ThreadPoolExecutor(max_workers=self._max_threads) as executor:
                ...
                # Wait for all MCP server initializations to complete
                    ...
                    [item[0] for item in futures_with_specs],
                    return_when=concurrent.futures.ALL_COMPLETED,
                )

                if not_done:
                    logger.warning(
                        "Some MCP server initializations were not done even though the 'wait for all' returned"
                    )
                ...
                for future, spec in futures_with_specs:
                    ...
                    try:
                        ...
                        spec["tool_names"] = [
                            getattr(
                                tool, "tool_name", getattr(tool, "__name__", str(tool))
                            )
                            for tool in tools
                        ]
                        ...
                        logger.debug(
                            "Successfully initialized MCP client: {}", client.server_id
                        )
                    except Exception as e:
                        logger.warning(
                            f"MCP client initialization failed for {getattr(client, 'server_id', 'unknown')}: {e}"
                        )
                        spec["error"] = str(e)
            ...
            logger.debug(
                "MCP server initialization completed for {} servers in {:.2f}ms",
                len(self._active_mcp_clients),
                init_time * 1000,
            )
        ...
        if logger.level("TRACE").no >= logger._core.min_level:
            logger.trace(
                "AgentProxy.__enter__ completed with {} active MCP servers",
                len(self._active_mcp_clients),
            )
        return self
    ...
    def _call_single_enter_safely(self, manager):
        ...
        """
        logger.trace(
            "_call_single_enter_safely called for manager type: {}",
    ...
    async def _handle_agent_stream(self, message: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Unexpected error in agent stream: {e}\nFull traceback:\n{tb_str}"
            )
            print(
    ...
    async def send_message_to_agent(
        self, message: str, show_user_input: bool = True
    ) -> bool:
        ...
        """
        logger.trace(
            "send_message_to_agent called with message type: {}, show_user_input: {}",
            type(message),
            show_user_input,
        )
        self._ensure_agent_available()
    ...
    def __call__(self, *args, **kwargs):
        ...
        if logger.level("TRACE").no >= logger._core.min_level:
            logger.trace(
                "__call__ called with {} args, {} kwargs", len(args), len(kwargs)
            )
        self._ensure_agent_available()
