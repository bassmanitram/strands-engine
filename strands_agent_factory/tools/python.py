...
def import_python_item(
    ...
    """
    logger.trace("import_python_item called with base_module='{}', item_sub_path='{}', package_path='{}', base_path='{}'", 
                base_module, item_sub_path, package_path, base_path)
    ...
    try:
        ...
        logger.debug("Loading item '{}' from module '{}' (base_path='{}', package_path='{}')", 
                    item_name, full_module_path, base_path, package_path)
    ...
    if package_path:
        ...
        item = _load_from_custom_path(full_module_path, item_name, package_path, base_path)
    else:
        logger.trace("Using standard import strategy")
        item = _load_from_standard_import(full_item_path, full_module_path, item_name)

    logger.trace("import_python_item completed successfully, returning item type: {}", type(item).__name__)
    return item


def _load_from_custom_path(full_module_path: str, item_name: str, package_path: str, base_path: Optional[str]) -> Any:
    """
...
def _load_from_standard_import(full_item_path: str, full_module_path: str, item_name: str) -> Any:
    """
    ...
    try:
        ...
        try:
            ...
            logger.debug("Successfully loaded item '{}' from parent module '{}'", item_name, full_module_path)
            return item
            
        except (ImportError, AttributeError) as e2:
            logger.error("Cannot load {}: parent module import failed with {}", full_item_path, e2)
