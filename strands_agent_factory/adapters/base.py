...
class FrameworkAdapter(ABC):
    ...
    def adapt_tools(self, tools: List[Tool], model_string: str) -> List[Tool]:
        ...
        if logger.level("TRACE").no >= logger._core.min_level:
            logger.trace(
                "FrameworkAdapter.adapt_tools called with {} tools, model_string='{}'",
        ...
        if tools:
            logger.debug(
                "FrameworkAdapter adapter: Tools passed through without modification"
            )
            logger.debug(
                "Note: Tool support varies across models. Model '{}' may have limited tool capabilities.",
                model_string,
            )
        else:
    ...
    def prepare_agent_args(
        ...
        **kwargs,
        ...
        if logger.level("TRACE").no >= logger._core.min_level:
            logger.trace(
                "FrameworkAdapter.prepare_agent_args called with system_prompt={}, messages={}, emulate_system_prompt={}, kwargs={}",
        ...
        if emulate_system_prompt and system_prompt:
            logger.debug(
                "Emulating system prompt by prepending to the first user message as requested."
            )
    ...
    def adapt_content(self, content: Messages) -> Messages:
        ...
        """
        logger.trace(
            "FrameworkAdapter.adapt_content called with content type: {}", type(content)
        )
        return content
    ...
    async def initialize(
        self, model: str, model_config: Optional[Dict[str, Any]] = None
    ) -> bool:
        """
        ...
        """
        logger.trace(
            "FrameworkAdapter.initialize called with model='{}', model_config={}",
            model,
            model_config,
        )
        logger.debug(
            "FrameworkAdapter.initialize: No initialization required, returning True"
        )
        return True
...
def load_framework_adapter(adapter_name: str) -> FrameworkAdapter:
    ...
    if adapter_name in FRAMEWORK_HANDLERS:
        ...
        try:
            ...
            raise AdapterError(
                f"Failed to load explicit adapter for {adapter_name}"
            ) from e
    ...
    if _can_handle_generically(adapter_name):
        ...
        try:
            ...
            raise AdapterError(
                f"Failed to create generic adapter for {adapter_name}"
            ) from e
...
def _load_explicit_adapter(adapter_name: str) -> FrameworkAdapter:
    ...
    try:
        ...
        logger.debug(
            "Successfully created explicit adapter: {}", type(adapter).__name__
        )
        return adapter
...
def _create_generic_adapter(framework_id: str) -> FrameworkAdapter:
    ...
    try:
        ...
        if not adapter:
            raise AdapterError(
                f"Generic adapter creation returned None for {framework_id}"
            )
        return adapter
    except ImportError as e:
        ...
        raise AdapterError(
            f"Failed to create generic adapter for {framework_id}"
        ) from e
