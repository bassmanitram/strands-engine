...
class TestAdapterSystemIntegration:
    ...
    def test_adapter_loading_priority_system(self):
        ...
        with patch(
            "strands_agent_factory.adapters.base._load_explicit_adapter"
        ) as mock_explicit:
            mock_adapter = Mock()
            ...
            framework_name = (
                list(FRAMEWORK_HANDLERS.keys())[0] if FRAMEWORK_HANDLERS else "litellm"
            )

            with patch(
                "strands_agent_factory.adapters.base.FRAMEWORK_HANDLERS",
                {framework_name: "test.adapter"},
            ):
    ...
    def test_adapter_loading_generic_fallback(
        self, mock_create_generic, mock_can_handle
    ):
    ...
    def test_adapter_error_propagation(self):
        ...
        with patch(
            "strands_agent_factory.adapters.base._can_handle_generically"
        ) as mock_can_handle:
            with patch(
                "strands_agent_factory.adapters.base._create_generic_adapter"
            ) as mock_create:
                mock_can_handle.return_value = True
                mock_create.side_effect = Exception("Generic adapter failed")

                with pytest.raises(
                    AdapterError, match="Failed to create generic adapter"
                ):
    ...
    def test_create_generic_adapter_error_handling(self, mock_adapter_class):
        ...
        with pytest.raises(
            Exception
        ):  # Should be wrapped in GenericAdapterCreationError
    ...
    def test_adapter_framework_name_consistency(self):
        ...
        with patch(
            "strands_agent_factory.adapters.generic.importlib.import_module"
        ) as mock_import:
            mock_model_class = Mock()
            mock_model_class.__annotations__ = {"model_id": str}
