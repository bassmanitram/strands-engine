...
class TestSummarizingConversationManager:
    ...
    def test_creates_agent_when_summarization_model_config_specified(self):
        ...
        with patch('strands_agent_factory.session.conversation.load_framework_adapter') as mock_load:
            mock_adapter = Mock()
            mock_model = Mock()
            ...
            with patch('strands_agent_factory.session.conversation.Agent') as mock_agent_class:
    ...
    def test_raises_error_when_agent_creation_fails_with_explicit_config(self):
        ...
        with patch('strands_agent_factory.session.conversation.load_framework_adapter') as mock_load:
            ...
            assert "Cannot fulfill explicit summarization configuration requirements" in str(exc_info.value)
    ...
    def test_fallback_when_agent_creation_fails_without_explicit_config(self):
        ...
        with patch('strands_agent_factory.session.conversation.load_framework_adapter') as mock_load:
            ...
            with patch('strands_agent_factory.session.conversation.SummarizingConversationManager') as mock_scm:
